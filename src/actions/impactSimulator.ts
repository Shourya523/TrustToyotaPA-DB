...
import { getDefaultDatabaseUri, DEFAULT_CONNECTION_ID, resolveDatabaseUri } from "@/src/lib/database-uri";

async function resolveUri(connectionId: string, userId?: string) {
  return resolveDatabaseUri(connectionId, userId);
}
...
export async function simulateSchemaChangeAction(
  ...
  try {
    ...
    if (columnName && connectionId !== DEFAULT_CONNECTION_ID) {
